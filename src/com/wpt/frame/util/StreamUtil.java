...
public class StreamUtil {
	...
	public static void readData(InputStream in,OutputStream out,Handler handler) throws Exception{
		if(in != null && out != null && handler != null){
			//发送消息开始下载音乐
//			handler.sendEmptyMessage(ConstantUtil.FILE_LOAD_START);
			//从服务器端读取音乐数据
			...
			while((len = bis.read(bytes)) != -1){
				...
				if(loaded%100 == 0){
					//发送的消息类型为正在下载中
//					Message msg = Message.obtain(handler, ConstantUtil.FILE_LOAD_UPDATE);
...
//					msg.sendToTarget();			
			...
			in.close();
			//发送消息。标识下载完成
//			handler.sendEmptyMessage(ConstantUtil.FILE_LOAD_END);
		}		
	}
	/**
	 * 获取文件流并将文件保存到指定的路径
